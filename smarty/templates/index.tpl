...
<form method="POST">
    ...
    <br>
    <p><label class="left-label" for="user_name">Ваше имя</label> <input name="user_name" type="text" id="user_name" value="{$item.user_name|escape:'htmlall'}">
        <br>
    ...
    <p><label class="left-label" for="add_name">Название объявления </label><input name="add_name" type="text" id="add_name" value="{$item.add_name|escape:'htmlall'}">
    <p><label class="left-label" for="add_description">Описание товара</label><textarea name="add_description" id="add_description" style="resize:none;">{$item.add_description|escape:'htmlall'}</textarea>
...
<table cellpadding="10px">
    ...
    {if !empty($data)}
        {foreach from=$data key=id item=i}
            <tr>
                <td>{$id}</td>
                <td><a href="../index.php?open={$id}">{$i.add_name}</a></td>
                <td>{$i.price}</td>
                <td>{$i.user_name}</td>
